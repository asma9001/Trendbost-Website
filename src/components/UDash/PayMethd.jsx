...
const PMethod = () => {
  ...
  useEffect(() => {
    const fetchPaymentMethods = async () => {
      ...
      try {
        ...
        setExistingMethods(response.data.user.paymentMethods || []);
      } catch (error) {
  ...
  const handleRemoveMethod = async (index) => {
    ...
    console.log(methodToRemove);

    try {
      // Assume methodToRemove contains the ID of the card to be removed
      await axiosInstance.delete(`/paymentMethod/deleteCard/${methodToRemove._id}`, {
        data: { userId },
      });

      // Update the state to remove the method from the UI
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      // Add the new method to the existing methods state with all details
      const newMethod = {
        paymentMethod: selectedMethod,
      ...
      // Reset the form data and selected method
      setFormData({});
      setSelectedMethod("");
      setShowNewMethods(false);
